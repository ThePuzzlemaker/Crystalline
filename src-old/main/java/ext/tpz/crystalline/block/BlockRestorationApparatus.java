...
public class BlockRestorationApparatus extends Block implements ITileEntityProvider, TOPInfoProvider {
    ...
    public void addProbeInfo(ProbeMode mode, IProbeInfo probeInfo, EntityPlayer player, World world, IBlockState state, IProbeHitData data) {
        ...
        if (te instanceof TERestorationApparatus) {
            TERestorationApparatus apparatus = (TERestorationApparatus) te;
            probeInfo.horizontal().item(new ItemStack(Items.CLOCK)).text(TextFormatting.GREEN + "Ticks: " + apparatus.getTimer() + "/1200");
            probeInfo.horizontal(probeInfo.defaultLayoutStyle().borderColor(0xffff0000)).item(new ItemStack(CrystallineBlocks.distillationBasin)).progress((calculate(apparatus.getTimer(), 1200)), 100, probeInfo.defaultProgressStyle().suffix("%")).horizontal();
            probeInfo.horizontal().item(apparatus.getEssenceStack()).text("Current Essence");
            probeInfo.horizontal().item(apparatus.getCrystalStack()).text("Current Crystal");
        }
    }
